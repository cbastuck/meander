 ...
 */
export function serveBoards(boardsDir) {
    var root = path.resolve(boardsDir);
    return {
        name: "serve-boards",
        configureServer: function (server) {
            server.middlewares.use("/boards", function (req, res, next) {
                var filePath = path.resolve(root, req.url.replace(/^\//, ""));
