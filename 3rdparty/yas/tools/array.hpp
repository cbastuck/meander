...
struct load_array_wrapper {
    load_array_wrapper(T** p, std::size_t *s, T*(*fn)(std::size_t)) 
    : ptr(p), size(s), alloc_fnptr(fn) {}
...
{
    load_array_wrapper<T> wrapper(ptr, size, allocfn);
    return wrapper; // return {ptr, size, allocfn};
}
