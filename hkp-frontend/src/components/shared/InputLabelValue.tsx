...
export default function InputLabelValue({ value: propValue, label: propLabel, onCommit, onFocus: _onFocus }: Props): JSX.Element {
  const [label, setLabel] = useState<string | null>(null);
  const [value, setValue] = useState<string | null>(null);
  const [editLabel, setEditLabel] = useState<string | null | undefined>(null);
  const [renamedLabelBuffer, setRenamedLabelBuffer] = useState<string | null>(null);
  ...
  return (
    <InputField
      ...
      value={value ?? ""}
      onChange={(newValue: string) => onCommit({ label: label ?? "", value: newValue })}
